...
zoom: 10.6,
regions: [
'千代田区', '中央区', '港区', '新宿区', '文京区', '台东区', '墨田区', '江东区',
'品川区', '目黑区', '大田区', '世田谷区', '涩谷区', '中野区', '杉并区', '丰岛区',
'北区', '荒川区', '板桥区', '练马区', '足立区', '葛饰区', '江户川区'
]
}
};
...
payload.shops = payload.shops
.map(normalizeLocationFields)
...
const country = city === 'Hong Kong' ? 'Hong Kong SAR' : (raw.country || CITIES[city].country);
const region = city === 'Tokyo'
? (validRegion(city, raw.region) ? raw.region : inferTokyoRegion(raw.address))
: raw.region;
...
const insideTokyo = Number.isFinite(latitude) && Number.isFinite(longitude)
&& latitude >= 35.45 && latitude <= 35.95
&& longitude >= 139.35 && longitude <= 140.10;
...
'涩谷区': ['渋谷区', '涩谷区', 'shibuya'],
'中野区': ['中野区', 'nakano'],
'杉并区': ['杉並区', '杉并区', 'suginami'],
'丰岛区': ['豊島区', '丰岛区', 'toshima'],
'北区': ['北区', 'kita'],
'荒川区': ['荒川区', 'arakawa'],
'板桥区': ['板橋区', '板桥区', 'itabashi'],
'练马区': ['練馬区', '练马区', 'nerima'],
'足立区': ['足立区', 'adachi'],
'葛饰区': ['葛飾区', '葛饰区', 'katsushika'],
'江户川区': ['江戸川区', '江户川区', 'edogawa']
};
...
sheet.innerHTML = `
<div class="sheet-backdrop" data-close-city></div>
<article class="sheet-card compact-card city-sheet-card">
<div class="sheet-grabber"></div>
<div class="sheet-title-row">
<div><p class="list-kicker">Cities</p><h2>选择城市</h2></div>
<button class="sheet-close" type="button" data-close-city aria-label="关闭">×</button>
</div>
<div id="cityList" class="city-list">
${Object.keys(CITIES).map(city => `
<button class="city-option${city === activeCity ? ' active' : ''}" type="button" data-city="${escapeHtml(city)}">
<span><strong>${escapeHtml(CITIES[city].label)}</strong><small>${escapeHtml(CITIES[city].country)}</small></span>
<b aria-hidden="true">${city === activeCity ? '✓' : ''}</b>
</button>`).join('')}
</div>
</article>`;
...
if (!rail) return;
rail.innerHTML = `
<button class="chip active" data-region="全部">全部 <span id="allCount">—</span></button>
${activeConfig.regions.map(region => `<button class="chip" data-region="${escapeHtml(region)}">${escapeHtml(region)}</button>`).join('')}
<button class="chip district-chip" id="districtButton" type="button">具体地区 <span>⌄</span></button>`;
}
...
regionLabel.querySelector('span').textContent = defaultCity === 'Tokyo' ? '行政区 *' : '大区 *';
form.dataset.multicityEnhanced = 'true';
fillRegionSelect(regionSelect, defaultCity);
cityLabel.querySelector('select').addEventListener('change', event => {
syncFormLocation(form, event.currentTarget.value, { clearRegion: true });
});
}
...
const inferredRegion = city === 'Tokyo' ? inferTokyoRegion(form.elements.address?.value) : currentRegion;
...
if (regionLabel) regionLabel.textContent = city === 'Tokyo' ? '行政区 *' : '大区 *';
}
function fillRegionSelect(select, city, selected = '') {
if (!select || !CITIES[city]) return;
select.innerHTML = '<option value="">请选择</option>'
+ CITIES[city].regions.map(region => `<option value="${escapeHtml(region)}">${escapeHtml(region)}</option>`).join('');
...
.map(city => `<option value="${escapeHtml(city)}"${city === selected ? ' selected' : ''}>${escapeHtml(CITIES[city].label)}</option>`)
.join('');
}
